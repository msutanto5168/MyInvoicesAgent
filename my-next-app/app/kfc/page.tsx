...
export default function KFCPage() {
  ...
  const [rows, setRows] = useState<Row[]>([
    { description: getDescription(getFirstOfNextMonth(today)), amount: 18510.20 },
  ...
  function getDescription(dueDateValue: string) {
    ...
    return `Rent for 98a Westall Road ${monthName} 01 - ${monthName} ${lastDay} ${year}`;
  }

  // Function to generate email subject based on due date
  const getEmailSubject = (dueDateValue: string) => {
    if (!dueDateValue) return "Invoice for Clayton South";

    const due = new Date(dueDateValue);
    const monthName = due.toLocaleString('default', { month: 'long' });
    const year = due.getFullYear();

    return `Invoice for ${monthName} ${year} - Clayton South`;
  };
  ...
  const getEmailBody = (
    ...
    return `Hi Southern Restaurants Group,
  ...
  * GST = $${gstValue.toFixed(2)}
  ...
  const handleGenerateInvoice = async () => {
    const API_URL = "https://api.invoiceagent.com.au/kfc-invoice";
  ...
  const handleSendEmail = async () => {
    const INVOICE_API_URL = "https://api.invoiceagent.com.au/kfc-invoice";
